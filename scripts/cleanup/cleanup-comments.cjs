...
function processFile(filePath) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      /eslint-|istanbul|@ts-ignore|@ts-expect-error|@preserve|copyright|license/i.test(
        line,
      )
    ) {
      out.push(line);
    ...
    if (
      /^\s*\/\/\s*(?:const |let |var |function |class |import |export |return |if\b|for\b|while\b|switch\b|try\b|await\b|interface\b|type\b|enum\b|new\s+|module\.|require\(|\w+\s*=>|<\w+)/i.test(
        line,
      )
    ) {
    ...
    if (
      /^\s*console\.(?:log|debug|info)\s*\(/.test(line) ||
      /^\s*debugger;?\s*$/.test(line)
    ) {
