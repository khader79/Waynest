...
for (const lang of ['es','de','zh','pt']) {
  ...
  for (const [k, ev] of Object.entries(enFlat)) {
    if ((k in flat) && flat[k] === ev && !isIgnored(ev)) {
      untranslated.push(k);
    }
  }
  console.log('=== ' + lang + ': ' + untranslated.length + ' truly untranslated ===');
  const sections = {};
  ...
  for (const [s, ks] of Object.entries(sections).sort((a,b) => b[1].length - a[1].length)) {
    console.log('  ' + s + ': ' + ks.length);
    if (ks.length <= 10) for (const k of ks) console.log('    ' + k + ' = "' + flat[k] + '"');
  }
  console.log();
}
