...
import { generateTripPlan } from '@/features/trip-planner/api';
...
export const useTripResults = () => {
  ...
  const loadSavedPlan = useCallback(async (planId) => {
    try {
      const { fetchTripPlanById } = await import('@/features/trip-planner/api');
  ...
  const addToWishlist = useCallback(async (placeId) => {
    try {
      const { addWishlistItem } = await import('@/modules/user/api');
