...
export const fetchTripPlanById = async (planId) =>
  get(ROUTES.trips.one(planId));

export const deleteTripPlan = async (planId) =>
  del(ROUTES.trips.remove(planId));

export const publishTripPlan = async (tripPlanId, payload) =>
  postJson(ROUTES.trips.share(tripPlanId), payload);

export const fetchPublicTripPlan = async (slug) =>
  get(ROUTES.trips.publicOne(slug));
