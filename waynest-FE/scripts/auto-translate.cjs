...
async function translateGemini(text, targetLanguageName) {
  ...
  if (!endpoint) {
    console.error(
      "GEMINI_API_URL not set — set to your Gemini endpoint (e.g. https://...): skipping Gemini.",
    );
    return null;
  }
  try {
    ...
    if (isGoogleGL) {
      ...
      url +=
        (url.includes("?") ? "&" : "?") + "key=" + encodeURIComponent(apiKey);
    } else {
...
async function main() {
  ...
  if (!mapPath) {
    console.error(
      "i18n-autogen-map.json not found. Run generate-autogen-keys first.",
    );
    process.exitCode = 2;
    return;
  }
  ...
  for (const lang of langs) {
    ...
    if (fs.existsSync(file)) {
      try {
        translationsByLang[lang] = readJSON(file);
      } catch (e) {
  ...
  for (let i = 0; i < map.length; i++) {
    ...
    report.push(row);
    if ((i + 1) % 20 === 0)
      console.log(`Processed ${i + 1}/${map.length} keys...`);
  }
  ...
  const reportJsonPath = path.join(
    wfRoot,
    "i18n-autogen-translated-report.json",
  );
  ...
  for (const r of report) {
    ...
    for (const lang of langs)
      cols.push(`"${(r.translations[lang] || "").replace(/"/g, '""')}"`);
    rows.push(cols.join(","));
  }
